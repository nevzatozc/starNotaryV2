...
it('can add the star name and star symbol properly', async() => {
    ...
    let instance = await StarNotary.deployed();
let user1 = accounts[1];
assert.equal("TBTKStar", await instance.name({from:user1}));
assert.equal("TBTK", await instance.symbol({from:user1}));
...
let user2 = accounts[2];
let starId = 8;
await instance.createStar("To be transfered to 2", starId,{from:user1});
...
assert.equal("Look up star", await instance.lookUptokenIdToStarInfo(starId));
});